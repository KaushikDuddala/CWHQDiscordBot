...
module.exports.message = (client, message) => {
	const prefix = '!';
	const aliases = ['definition', 'def', 'identify', 'dictionary', 'meaning'];
			...
			let embed = {
				...
				fields: result.meanings.map(meaning => {
					return {
						...
						value: `**Definition:** ${meaning.definitions[0].definition}${meaning.definitions[0].example ? `\n**Example:** ${meaning.definitions[0].example.replace(word, `**${word}**`)}` : ''}${meaning.definitions[0].synonyms ? `\n**Synonyms:** ${meaning.definitions[0].synonyms.join(', ')}` : ''}`
					}
				})
			}
